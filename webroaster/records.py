...
def model_list(request, model_name):
    ...
    if model_name == "documents":
        return render_page(request, "webroaster/document_list.html", document_register_context(), model_name)
    if model_name == "attendance":
        ...
        return render_page(request, "webroaster/attendance_roster.html", context, model_name)
    ...
    return render_page(request, "webroaster/model_list.html", context, model_name)
...
def model_import(request, model_name):
    ...
    if config.get("managed_table"):
        ...
        return redirect("webroaster:list", model_name=model_name)
    ...
    return render_page(request, "webroaster/model_import.html", context, model_name)
...
def model_detail(request, model_name, pk):
    ...
    if model_name == "incidents":
        ...
        return render_page(request, "webroaster/incident_detail.html", context, model_name)
    return render_page(request, "webroaster/model_detail.html", context, model_name)
...
def model_create(request, model_name):
    ...
    if model_name == "incidents":
        ...
        if request.method == "POST" and form.is_valid():
            ...
            return redirect("webroaster:detail", model_name=model_name, pk=incident.pk)
        context = {
            "model_name": model_name,
            "title": "Add Incident",
            "form": form,
    ...
    if model_name == "leaves":
        ...
        if request.method == "POST" and form.is_valid():
            ...
            return redirect("webroaster:detail", model_name=model_name, pk=leave.pk)
        context = {
            "model_name": model_name,
    ...
    if model_name == "assets":
        ...
        if request.method == "POST" and form.is_valid():
            ...
            return redirect("webroaster:detail", model_name=model_name, pk=asset.pk)
        context = {
            "model_name": model_name,
            "title": "Add Asset",
            "form": form,
        ...
        return render_page(request, "webroaster/asset_form.html", context, model_name)
    if model_name == "invoices":
        ...
        if request.method == "POST" and form.is_valid() and invoice_items_are_valid:
            ...
            if invoice_mode == "split_sites":
                ...
                return redirect("webroaster:detail", model_name=model_name, pk=invoice.pk)
        context = {
            "model_name": model_name,
            "title": "Add Invoice",
            "form": form,
        ...
        if request.method == "POST":
            ...
            if form_is_valid and item_formset_is_valid:
                try:
                    ...
                    return redirect("webroaster:detail", model_name=model_name, pk=proforma.pk)
        context = {
            "model_name": model_name,
    ...
    if model_name == "deployments":
        ...
        if request.method == "POST" and form.is_valid():
            ...
            return redirect("webroaster:detail", model_name=model_name, pk=deployment.pk)
        context = {
            "model_name": model_name,
    ...
    if model_name == "performance-evaluations":
        ...
        if request.method == "POST" and form.is_valid():
            ...
            return redirect("webroaster:detail", model_name=model_name, pk=evaluation.pk)
        context = {
            "model_name": model_name,
    ...
    if model_name == "documents":
        ...
        return render_page(request, "webroaster/document_upload.html", context, model_name)
    ...
    if request.method == "POST" and entry_formset.is_valid():
        ...
        if objects:
            ...
            if len(objects) == 1:
                return redirect("webroaster:detail", model_name=model_name, pk=objects[0].pk)
            return redirect("webroaster:list", model_name=model_name)
...
def model_update(request, model_name, pk):
    ...
    if request.method == "POST":
        ...
        if form_is_valid and formset_is_valid and invoice_items_are_valid and proforma_items_are_valid:
            ...
            return redirect("webroaster:detail", model_name=model_name, pk=obj.pk)
...
def model_delete(request, model_name, pk):
    ...
    return render_page(request, "webroaster/model_confirm_delete.html", context, model_name)
