...
class LeaveReviewWorkflowTests(TestCase):
    ...
    def test_leave_submission_creates_actionable_notifications(self):
        ...
        response = self.client.post(reverse("webroaster:leave_notification_action", args=[verifier_notification.pk, "verify"]))
        self.assertEqual(response.status_code, 302)
        leave.refresh_from_db()
        self.assertEqual(leave.operations_verification_status, "verified")
        ...
        response = self.client.post(reverse("webroaster:leave_notification_action", args=[approver_notification.pk, "approve"]))
        self.assertEqual(response.status_code, 302)
        leave.refresh_from_db()
        self.assertEqual(leave.approval_status, "approved")
...
class PayrollProcessingTests(TestCase):
    ...
    def test_supervisor_salary_is_attendance_based(self):
        region = Region.objects.create(region_name="Payroll Region")
        site = Site.objects.create(
            region=region,
            site_name="Payroll Site",
            site_address="Kampala",
            day_shift_guards=1,
            night_shift_guards=0,
        )
        shift = Shift.objects.create(start_time=time(7, 0), end_time=time(18, 0), hours_per_shift=0)
        supervisor = self.make_employee("SupervisorAttendance", "supervisor")
        ...
        Attendance.objects.create(
            ...
            site=site,
            shift=shift,
            scheduled_guard=supervisor,
            attended_guard=supervisor,
            present=True,
            employee=supervisor,
        ...
        self.assertFalse(supervisor.uses_fixed_monthly_salary)
        self.assertEqual(salary.shifts_worked, 1)
        self.assertEqual(salary.cumulative_shifts_worked, 1)
        self.assertEqual(salary.basic_salary, Decimal("30000.00"))
        self.assertEqual(salary.overtime_pay, Decimal("0.00"))

    def test_attendance_paid_staff_use_latest_attendance_month(self):
        region = Region.objects.create(region_name="July Payroll Region")
        site = Site.objects.create(
            region=region,
            site_name="July Payroll Site",
            site_address="Kampala",
            day_shift_guards=1,
            night_shift_guards=0,
        )
        ...
        self.assertEqual(salary.period_start_date, date(2026, 7, 1))
        self.assertEqual(salary.period_end_date, date(2026, 7, 31))
        self.assertEqual(salary.shifts_worked, 1)
    ...
    def test_paye_uses_july_2026_resident_rates(self):
        ...
        salary = employee.salary
        salary.allowances = Decimal("0.00")
        salary.bonus = Decimal("0.00")
        salary.deductions = Decimal("0.00")
        salary.basic_salary = Decimal("500000.00")
        salary.overtime_pay = Decimal("0.00")

        self.assertEqual(salary.paye_tax_year_basis, "Uganda resident PAYE rates effective 1 July 2026")
        self.assertEqual(salary.paye, Decimal("38250.00"))
...
class PasswordManagementTests(TestCase):
    ...
    def test_superuser_can_open_password_management(self):
        ...
        response = self.client.get(reverse("webroaster:password_management"))
    ...
    def test_superuser_can_activate_and_reset_password(self):
        ...
        activate_response = self.client.post(
            reverse("webroaster:password_management_action", args=[self.staff_user.pk]),
            {"action": "activate"},
        )
        self.assertRedirects(activate_response, reverse("webroaster:password_management"))
        ...
        self.assertRedirects(reset_response, reverse("webroaster:password_management"))
        self.staff_user.refresh_from_db()
    ...
    def test_bulk_deactivates_users_linked_to_terminated_staff(self):
        ...
        response = self.client.post(
            reverse("webroaster:password_management"),
            {"action": "deactivate_terminated"},
        )

        self.assertRedirects(response, reverse("webroaster:password_management"))
        self.staff_user.refresh_from_db()
