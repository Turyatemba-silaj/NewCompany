...
class Salary(models.Model):
    ...
    def is_attendance_paid_employee(self):
        return self.employee_id and self.employee.role in {"guard", "supervisor"}

    def latest_payable_attendance_date(self):
        if not self.employee_id:
            return None
        attendances = Attendance.objects.select_related("shift").filter(
            attended_guard=self.employee,
            present=True,
        ).order_by("-date", "-attendance_id")
        for attendance in attendances:
            if attendance.is_payable_shift:
                return attendance.date
        return None

    def get_period_dates(self):
        today = self.latest_payable_attendance_date() if self.is_attendance_paid_employee() else timezone.localdate()
        if today is None:
    ...
    def shifts_worked(self):
        ...
        return self.attendance_shift_count(start_date=start_date, end_date=end_date)

    def attendance_shift_count(self, start_date=None, end_date=None):
        if not self.employee_id:
            return 0
    ...
    @property
    def cumulative_shifts_worked(self):
        return self.attendance_shift_count()
    ...
    @property
    def paye_tax_year_basis(self):
        return "Uganda resident PAYE rates effective 1 July 2026" if self.period_end_date >= timezone.datetime(2026, 7, 1).date() else "Uganda resident PAYE rates before 1 July 2026"

    @property
    def nssf_basis(self):
        return "NSSF: employee 5% and employer 10% of gross pay"

    def calculate_resident_paye_2026(self, taxable):
        if taxable <= Decimal("335000.00"):
            return Decimal("0.00")
        if taxable <= Decimal("410000.00"):
            return ((taxable - Decimal("335000.00")) * Decimal("0.20")).quantize(Decimal("0.01"))
        if taxable <= Decimal("485000.00"):
            return (Decimal("15000.00") + ((taxable - Decimal("410000.00")) * Decimal("0.25"))).quantize(Decimal("0.01"))
        paye = Decimal("33750.00") + ((taxable - Decimal("485000.00")) * Decimal("0.30"))
        if taxable > Decimal("10000000.00"):
            paye += (taxable - Decimal("10000000.00")) * Decimal("0.10")
        return paye.quantize(Decimal("0.01"))

    def calculate_resident_paye_legacy(self, taxable):
    ...
    def paye(self):
        ...
        if self.period_end_date >= timezone.datetime(2026, 7, 1).date():
            return self.calculate_resident_paye_2026(taxable)
        return self.calculate_resident_paye_legacy(taxable)
