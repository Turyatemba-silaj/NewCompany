...
def duty_roster_export(request):
    ...
    return render_page(request, "webroaster/duty_roster_export.html", context, "deployments")
...
def duty_roster_upload(request):
    ...
    if request.method == "POST" and form.is_valid():
        ...
        if saved_count:
            ...
            return redirect("webroaster:list", model_name="deployments")
    ...
    return render_page(request, "webroaster/duty_roster_upload.html", context, "deployments")


def checklist_deployments(deployment_area, shift_type, work_date):
    deployments = (
        Deployment.objects.select_related("client", "site", "site__region", "shift")
        .filter(site__region=deployment_area, status="active", start_date__lte=work_date)
        .filter(Q(end_date__isnull=True) | Q(end_date__gte=work_date))
        .order_by("site__site_name", "start_date", "deployment_id")
    )
    return [deployment for deployment in deployments if shift_type in deployment.covered_shift_types]


def checklist_area_guards(deployment_area, work_date):
    return list(
        Employee.objects.filter(
            role="guard",
            status="active",
            deployment_areas__region=deployment_area,
            deployment_areas__status="active",
            deployment_areas__start_date__lte=work_date,
        )
        .filter(Q(deployment_areas__end_date__isnull=True) | Q(deployment_areas__end_date__gte=work_date))
...
def build_supervisor_checklist_rows(deployment_area, shift_type, work_date):
    deployments = checklist_deployments(deployment_area, shift_type, work_date)
    area_guard_ids = {guard.pk for guard in checklist_area_guards(deployment_area, work_date)}
    ...
    if deployments:
        attendance_records = (
            Attendance.objects.select_related("scheduled_guard", "attended_guard", "shift")
            .filter(
                deployment__in=deployments,
                date=work_date,
                shift__shift_type__in=("day", "night"),
                scheduled_guard__role="guard",
                scheduled_guard__status="active",
                scheduled_guard_id__in=area_guard_ids,
            )
            .order_by("shift__shift_type", "scheduled_guard__employee_number", "scheduled_guard__first_name", "scheduled_guard__last_name")
        )
        for attendance in attendance_records:
            record_shift_type = attendance.shift.shift_type if attendance.shift_id else ""
            attendance_by_deployment.setdefault(attendance.deployment_id, {}).setdefault(record_shift_type, []).append(attendance.scheduled_guard)

    for deployment in deployments:
        shift_allocations = {}
        site_guards = [
            guard
            for guard in site_roster_staff(deployment.site, work_date)
            if guard.pk in area_guard_ids and guard.role == "guard"
        ]
        deployment_attendance = attendance_by_deployment.get(deployment.pk, {})

        for covered_shift_type in deployment.covered_shift_types:
            required_count = deployment.site.day_shift_guards if covered_shift_type == "day" else deployment.site.night_shift_guards
            if required_count <= 0:
                shift_allocations[covered_shift_type] = []
                continue

            saved_shift_guards = []
            for guard in deployment_attendance.get(covered_shift_type, []):
                if guard.pk not in scheduled_guard_ids or covered_shift_type == shift_type:
                    saved_shift_guards.append(guard)

            if saved_shift_guards:
                allocated_guards = saved_shift_guards if covered_shift_type == shift_type else saved_shift_guards[:required_count]
            else:
                allocated_guards = rotating_available_guards(
                    site_guards,
                    required_count,
                    work_date,
            ...
            for guard in allocated_guards:
                scheduled_guard_ids.add(guard.pk)
            shift_allocations[covered_shift_type] = allocated_guards

        scheduled_guards = shift_allocations.get(shift_type, [])
        required_count = deployment.site.day_shift_guards if shift_type == "day" else deployment.site.night_shift_guards
        if required_count <= 0:
            continue
        for guard in scheduled_guards:
            rows.append(
                {
                    ...
                    "guard": guard,
                    "employee_number": guard.employee_number or guard.pk,
                    "phone_number": guard.phone_number,
                }
            )
        shortage_count = max(required_count - len(scheduled_guards), 0)
        if shortage_count:
            shortage_rows.append(
                {
                    "site": deployment.site,
                    "required_count": required_count,
                    "available_count": len(scheduled_guards),
...
def write_supervisor_checklist_csv(response, checklist_rows, shortage_rows, selected):
    writer = csv.writer(response)
    writer.writerow(["Supervisor Duty Checklist"])
    writer.writerow(["Deployment Area", selected["deployment_area"]])
    writer.writerow(["Shift Type", selected["shift_label"]])
    writer.writerow(["Duty Date", selected["checklist_date"]])
    writer.writerow([])
    writer.writerow([
        "No.",
        "Site",
    ...
    if shortage_rows:
        writer.writerow([])
        writer.writerow(["Shortages"])
        writer.writerow(["Site", "Required", "Available", "Shortage"])
        for row in shortage_rows:
            writer.writerow([row["site"], row["required_count"], row["available_count"], row["shortage_count"]])


def supervisor_checklist(request):
    require_view_access(request, "supervisor_checklist")
    form = SupervisorDutyChecklistForm(
        request.GET or None,
        initial={"checklist_date": timezone.localdate()},
    )
    checklist_rows = []
    shortage_rows = []
    deployments = []
    ...
    if form.is_valid():
        deployment_area = form.cleaned_data["deployment_area"]
        shift_type = form.cleaned_data["shift_type"]
        checklist_date = form.cleaned_data["checklist_date"]
        checklist_rows, shortage_rows, deployments = build_supervisor_checklist_rows(
            deployment_area,
            shift_type,
            checklist_date,
        )
        ...
        if request.GET.get("export") == "excel":
            filename_area = re.sub(r"[^A-Za-z0-9_.-]+", "-", str(deployment_area)).strip("-")
            filename = f"supervisor-checklist-{filename_area}-{shift_type}-{checklist_date:%Y%m%d}.csv"
            response = HttpResponse(content_type="text/csv")
            response["Content-Disposition"] = f'attachment; filename="{filename}"'
            write_supervisor_checklist_csv(response, checklist_rows, shortage_rows, selected)
            return response

    context = {
        "title": "Supervisor Duty Checklist",
        "form": form,
...
def asset_store_report(request):
    ...
    return render_page(request, "webroaster/asset_store_report.html", context, "assets")
...
def asset_group_detail(request):
    ...
    if not assets:
        ...
        return redirect("webroaster:list", model_name="assets")
    ...
    return render_page(request, "webroaster/asset_group_detail.html", context, "assets")
...
def asset_issued_out_detail(request):
    ...
    return render_page(request, "webroaster/asset_issued_out_detail.html", context, "assets")
...
def contract_report(request, pk):
    ...
    return render_page(request, "webroaster/contract_report.html", context, "contracts")
    ...
    if regions.exists():
        guards = (
            Employee.objects.filter(
                ...
                deployment_areas__status="active",
                deployment_areas__start_date__lte=timezone.localdate(),
            )
            .filter(Q(deployment_areas__end_date__isnull=True) | Q(deployment_areas__end_date__gte=timezone.localdate()))
            .distinct()
            .order_by("employee_number", "first_name", "last_name")
        )
    return JsonResponse(
...
def incident_manage(request, pk):
    ...
    if request.method == "POST" and form.is_valid():
        ...
        return redirect("webroaster:incident_manage", pk=incident.pk)
    ...
    return render_page(request, "webroaster/incident_manage.html", context, "incidents")
...
def incident_investigation_report(request, pk):
    ...
    return render_page(request, "webroaster/incident_investigation_report.html", context, "incidents")
...
def incident_notifications(request):
    ...
    return render_page(request, "webroaster/incident_notifications.html", context, "incidents")
...
def incident_report(request):
    ...
    return render_page(request, "webroaster/incident_report.html", context, "incidents")
